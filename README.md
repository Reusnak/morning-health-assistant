...
一个基于 Hermes 架构（感知→思考→行动→记忆）的晨间健康助手，通过 DeepSeek LLM 根据用户的健康数据、天气和日程动态生成个性化问候，支持多轮对话、目标追踪和定时调度。
...
思考 (Reasoning)   → 规则引擎评分 → LLM 生成个性化问候/追问/鼓励
行动 (Action)      → 彩色终端多轮对话
记忆 (Memory)      → 自动提取目标 + 保存交互记录
```
